...
El bot guarda todo en una base de datos D1 en Cloudflare. **`<DB>` en los comandos
de abajo NO es literal**: cada bot tiene su propia base con un nombre único
(`panaclaw_<giro>_<uid>_db`), así que sácalo del `wrangler.toml` de ese bot con
`grep database_name wrangler.toml` antes de correr nada. Tú la
