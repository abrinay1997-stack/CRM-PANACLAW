...
| `D1 create ... already exists` | la base de datos ya existía | corre `pnpm wrangler d1 list`, copia el `database_id` real y pégalo en `wrangler.toml` (binding **DB**; el nombre está en `database_name` del `wrangler.toml`) |
...
```bash
# Los nombres salen del wrangler.toml de ESTE bot — no los inventes:
grep -E 'database_name|index_name|bucket_name' wrangler.toml

pnpm wrangler d1 create <database_name del toml>
pnpm wrangler vectorize create <index_name del toml> --dimensions=1024 --metric=cosine
pnpm wrangler r2 bucket create <bucket_name del toml>
```
...
| deploy-check: `Missing binding DB / KB / CATALOG` | la base de datos, el índice o el bucket no existen | saca los nombres con `grep -E 'database_name\|index_name\|bucket_name' wrangler.toml` y crea el faltante con ESE nombre (Vectorize y R2 se enlazan por nombre, no por id) |
