...
function stampBotConfig(dir, plan, slug) {
  ...
  // BOT_TIMEZONE: misma mecánica de reemplazar-o-insertar que BOT_NICHE, y por
  // el mismo motivo. Es la var que decide a qué hora se agenda: sin ella el
  // runtime cae a America/Mexico_City, así que un bot de Panamá o de Madrid
  // agendaba con la hora corrida. La región ya sabe la zona; solo hay que
  // escribirla donde el Worker la mira, que es [vars] y no member/.
  const tz = (REGIONS[REGION] || REGIONS["es-419"]).tz;
  if (/BOT_TIMEZONE\s*=\s*"[^"]*"/.test(s)) {
    s = s.replace(/BOT_TIMEZONE\s*=\s*"[^"]*"/g, `BOT_TIMEZONE = "${tz}"`);
  } else {
    s = s.replace(/^\[vars\][^\n]*\n/m, (m) => `${m}BOT_TIMEZONE = "${tz}"\n`);
  }
...
export const memberConfig = {
  ...
  // NOTA PARA EL DUEÑO, no configuración: hoy nada del Worker lee memberConfig
  // (solo businessConfig, vía src/businessContext.ts). Quedan aquí porque el
  // init ya los sabe y sirven de referencia al llenar el panel.
  //
  // Lo que SÍ decide la zona horaria de las citas es la var BOT_TIMEZONE del
  // wrangler.toml (o CALCOM_TIMEZONE, que manda sobre ella). El init la estampa
  // con este mismo valor, así que cambiarla aquí no basta: cámbiala allá.
  timezone: ${j(R.tz)},
  // Símbolo con el que quieres que se hablen los precios. Es una nota: el bot
  // usa el que esté escrito en el conocimiento y en el contexto del negocio.
