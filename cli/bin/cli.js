...
const REPO = process.env.PANACLAW_REPO || "Richardlovelove/CRM-PANACLAW";
...
function stampBotConfig(dir, plan, slug) {
  ...
  // así que va vacía: `panaclaw pair` la escribe tras el deploy y el runtime cae a su
...
async function cmdDoctor(dirArg, flags) {
  ...
  // 7) pairing con el dashboard de panaclaw.com (solo si `pair` ya corrió)
  if (marker.paired) {
    if (baseUrl) {
      try {
        ...
        if (r.status === 200 || r.status === 401) ok("Dashboard panaclaw.com: conectado");
        else if (r.status === 404) warn("Dashboard panaclaw.com: el bot no trae la API de pairing", "Actualiza y reconecta: npx panaclaw update · npx panaclaw pair");
        else warn(`Dashboard panaclaw.com: el bot respondió HTTP ${r.status}`, "Reintenta: npx panaclaw pair");
      } catch { warn("Dashboard panaclaw.com: el bot no respondió", "¿Sigue desplegado? Reintenta: npx panaclaw pair"); }
    } else warn("Dashboard panaclaw.com: vinculado pero sin DASHBOARD_BASE_URL", "No pude probar la conexión.");
  }
