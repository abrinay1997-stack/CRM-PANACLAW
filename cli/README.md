...
> Parte de [PanaClaw](https://panaclaw.com), la plataforma de chatbots de PanaClaw.
...
```bash
# 1 · asistente: elige idioma, licencia (gratis o PanaClaw+) y giro
npx panaclaw init
...
| Comando | Qué hace |
|---|---|
| `panaclaw init` | Asistente interactivo: idioma (ES/EN), licencia (gratis con tu correo o key `HZN-…`), elige el giro e instala. |
| `panaclaw list` | Muestra el catálogo de bots disponibles para tu plan. |
| `panaclaw install <slug>` | Instala un giro específico (p. ej. `restaurante`, `barberia`, `inmobiliaria`). |
| `panaclaw update [carpeta]` | Trae la versión nueva **conservando** tu `member/` (config, base de conocimiento). |
| `panaclaw doctor [carpeta]` | Diagnóstico del bot instalado: versión, archivos, licencia y si el worker responde. |

Opciones útiles:

- `--key HZN-XXXX-XXXX-XXXX` — pasa tu licencia de PanaClaw+ sin el asistente (`install` / `update` / `init`).
...
¿No tienes licencia de PanaClaw+? La obtienes al **entrar a la comunidad de PanaClaw**
([panaclaw.com](https://panaclaw.com)): ahí recibes tu llave `HZN-…`.

La licencia y el idioma se guardan en `~/.panaclaw/config.json`. La versión instalada
vive en el marcador `.panaclaw.json` dentro de la carpeta de tu bot.
...
- `superpoderes` (enciende y configura los 12), `reportes` (reporte diario con tu marca), `conexiones-composio` (conecta apps: Gmail, Slack…), `voz-de-marca`, `mantenimiento`, `afinar`, `campaña`, `clonar`, `precios` — operación y ajustes. *(PanaClaw+)*
- **Modo Agencia:** `cliente-nuevo`, `cliente-misterioso` (demo para un prospecto), `roi`, `cotizar`, `propuesta`, `cobrar` — para revender. *(PanaClaw+)*
...
- **PanaClaw +** — los 14 giros con sus paneles, los comandos recurrentes y el Modo Agencia.
  Se activa con una key `HZN-…`. → [Únete a PanaClaw +](https://panaclaw.com)
...
**https://panaclaw.com/docs/**

## Privacidad

`panaclaw` corre en tu máquina. No sube tu código ni los datos de tus clientes a
PanaClaw: solo valida tu licencia y baja el bot desde el control plane. El bot y
todos sus datos viven en **tu** Cloudflare, con **tus** llaves.

---

© PanaClaw · [panaclaw.com](https://panaclaw.com)
...
|---|---|---|
| **Red** (`node:http`) | Levanta un servidor **local en 127.0.0.1** que recibe el regreso del navegador al hacer `panaclaw login`. | Solo escucha en tu propia máquina, en un puerto temporal, y se cierra al terminar. Las llamadas a internet van por `fetch` a nuestro servidor de licencias. |
...
| **Variables de entorno** | Únicamente las suyas: `PANACLAW_SERVER`, `PANACLAW_CLOUD`, `PANACLAW_GET_URL`, `PANACLAW_YES`, `PANACLAW_NO_ART`, `PANACLAW_NO_BROWSER`, `PANACLAW_NO_AGENT_SKILL`, `PANACLAW_KEY`, `PANACLAW_SERVER`, y `NO_COLOR` (estándar). | **No lee ninguna credencial del sistema.** Nada de tokens de nube, claves de npm ni variables ajenas. |
| **Sistema de archivos** | Escribe la carpeta del bot que instalas y tu configuración en `~/.panaclaw/`. | Nada fuera de eso. |
| **Cadenas URL** | Los dominios propios: panaclaw.com y el servidor de licencias. | No hay direcciones IP ni dominios de terceros. |
